...
tag="SYNTH5"
seed=4
subsetSample=Sample4
samples=(Sample1 Sample2 Sample3 Sample4 Sample5)



cat ${TSVbarcodesPath}/*barcodes.tsv | sort | uniq -c | awk '$1 > 1' | awk '{print $2}' > blacklist.txt

for i in "${samples[@]}"
do
zcat ${TSVbarcodesPath}/${i}/outs/filtered_feature_bc_matrix/barcodes.tsv.gz | grep -v -f blacklist.txt | sort -R >  $i.goodBarcodes.tsv
done


head -n 2000 $subsetSample.goodBarcodes.tsv > temp && mv temp $subsetSample.goodBarcodes.tsv
...
cat *.goodBarcodes.ID.tsv > Barcodes-IDs.tsv
